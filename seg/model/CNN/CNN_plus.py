...
class UNetISDNetHybrid(torch.nn.Module):
    def __init__(self, n_channels, n_classes, patch_size, bilinear=True):
        super(UNetISDNetHybrid, self).__init__()
...
class UNet_plain(torch.nn.Module):
    def __init__(self, n_channels, n_classes, patch_size, bilinear=True):
        super(UNet_plain, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.patch_size = patch_size 
...
if __name__ == '__main__':
    ...
    model = UNetISDNetHybrid(n_channels=3, n_classes=1, patch_size=16).cuda()
    summary(model, (3, 256, 256), batch_size=10)