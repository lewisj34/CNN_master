...
class VisionTransformer(nn.Module):
    ...
    def forward(self, im, return_features=True):
        ...
        # print(f'pos_embed.shape before reshaping: {pos_embed.shape}')
        ...
        # print(f'pos_embed.shape after reshaping: {pos_embed.shape}')
        x = x + pos_embed
        x = self.dropout(x)

        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x) # just normalizes, no dim change so for example input of [2, 3, 256, 256] output for this specific layer would be [2, 257, 768]
