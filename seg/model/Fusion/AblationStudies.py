...
from seg.model.zed.AblationStudiesZedNet import zedNetDWSepWithCCMinAllOfItNoPSEncoder, zedNetNoPSDecoder
...
class NewZedFusionNetworkDWSepWithCCMinDWModuleInEveryUpDownModuleNoPSEnc(nn.Module):
    def __init__(
        self, 
        cnn_model_cfg,
        trans_model_cfg,
        ...
        super(NewZedFusionNetworkDWSepWithCCMinDWModuleInEveryUpDownModuleNoPSEnc, self).__init__()
        ...
        self.cnn_branch = zedNetDWSepWithCCMinAllOfItNoPSEncoder(
            n_channels_in=trans_model_cfg['num_output_trans'],
...
class NewZedFusionNetworkDWSepWithCCMinDWModuleInEveryUpDownModuleNoTransEnc(nn.Module):
    def __init__(
        self, 
        cnn_model_cfg,
        trans_model_cfg,
        ...
        super(NewZedFusionNetworkDWSepWithCCMinDWModuleInEveryUpDownModuleNoTransEnc, self).__init__()
...
running the terminal right now so...') # SEE BELOW.... decoder = 'linear'
        # need to do something or pull information from the trans_model_cfg and
        #  pull that info. but yeah. wahtever rn lol 
        self.trans_branch = create_transformerNoTransEncoder(
            trans_model_cfg, 
            'linear', 
            self.cnn_branch.x_1_16.shape[1],
        )
    ...
    def forward(self, images):
        ...
        x_final_trans = self.trans_branch(self.cnn_branch.x_1_16) # 5 x 1 x 156 x 156
...
# BEST RESULTS -> in results/DataParallel/DataParallel_11/current_checkpoints/DataParallel-218.pth on dlearn6
class NewZedFusionNoDecodersNoMergers(nn.Module):
    def __init__(
        self, 
        cnn_model_cfg,
        trans_model_cfg,
        ...
        super(NewZedFusionNoDecodersNoMergers, self).__init__()
        ...
        self.cnn_branch = zedNetNoPSDecoder(
            n_channels=cnn_model_cfg['in_channels'],
            n_classes=cnn_model_cfg['num_classes'],
            patch_size=cnn_model_cfg['patch_size'],
            bilinear=True,
...
running the terminal right now so...') # SEE BELOW.... decoder = 'linear'
        # need to do something or pull information from the trans_model_cfg and
        #  pull that info. but yeah. wahtever rn lol 
        self.trans_branch = create_transformerNoTransDecoder(trans_model_cfg, 
            decoder='linear')
...
class NewZedFusionWithMergersButNoDecoders(nn.Module):
    def __init__(
        self, 
        cnn_model_cfg,
        trans_model_cfg,
        ...
        super(NewZedFusionWithMergersButNoDecoders, self).__init__()
