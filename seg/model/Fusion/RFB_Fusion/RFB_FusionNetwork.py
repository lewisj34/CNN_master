...
class RFBFusionModule(nn.Module):
    def __init__(
        self,
        in_chan_t,
        in_chan_c,
        ...
        self.conv = BNRconv3x3(inter_chan, out_chan, dilation=dilation)

    
    def forward(self, x_t, x_c, x_extra=None):
        x_t = self.rfb_t(x_t)
        x_c = self.rfb_c(x_c)

        if x_extra == None:
            assert isinstance(x_extra, list), \
                f'Additional tensors should be inserted as list in x_extra param'
