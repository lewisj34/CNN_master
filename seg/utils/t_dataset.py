...
# FOR image_test.py -> includes originalTxtFile which stores the original file path as well 
class tDatasetImageTest(data.Dataset):
    """
    TESTING AND VALIDATION DATASET. FINAL VERSION. 
    ...
    def __init__(
        ...
        normalization="vit",
        normalize_gt=False,
        originalTxtFile=None,
    ):
        assert originalTxtFile != None, f'Input originalTxtFile arg'
        ...
        if originalTxtFile != None:
            with open(originalTxtFile) as file:
                lines = file.readlines()
                lines = [line.rstrip() for line in lines]
            self.originalTxtFile = lines
    ...
    def __getitem__(self, index):
        ...
        image = self.img_transform(image)
        gt = self.gt_transform(gt)
        return image, gt, self.originalTxtFile[index]

    def __len__(self):
        return self.size

def get_tDatasetImageTest(
    image_root,
    gt_root, 
    normalize_gt=False,
    batch_size=1,
    ...
    GETTER FUNCTION FOR TEST AND VALIDATION DATASETS 
        ...
        the transfer between the CPU and GPU later. 
    """
    dataset = tDatasetImageTest(
        image_root = image_root,
        gt_root = gt_root,
        normalization = normalization,
        normalize_gt = normalize_gt,
        originalTxtFile = originalTxtFile,
    )
    data_loader = data.DataLoader(
        dataset=dataset,
        batch_size = batch_size,
...
def get_tDatasets_masterImageTest(
    save_dir,
    normalize_gt=False,
    batch_size=1, 
    normalization='vit',
