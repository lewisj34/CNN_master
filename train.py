...
def main(
    ...
    ****************************** USAGE ****************************** 
    ...
    if model_name == 'basic':
        ...
        exit(1)
    elif model_name == "NewZedFusionNetworkMODPWOut":
        cnn_model_cfg["num_output_channels_cnn"] = [64 // 2, 128 // 2, 256, 512, 512, 256, 128, 64 // 2, 64 // 2]
        trans_model_cfg["trans_decoder_inter_chans"] = [128, 32, 16, 8]
        from seg.model.Fusion.modZedFusion import NewZedFusionNetworkMODPWOut
        model = NewZedFusionNetworkMODPWOut(
            cnn_model_cfg,
            trans_model_cfg,
            num_output_channels_cnn=cnn_model_cfg["num_output_channels_cnn"],
            trans_decoder_inter_chans=trans_model_cfg["trans_decoder_inter_chans"],
    ...
    elif model_name == 'new_fusion_zed_dwsep_rfb':
        from seg.model.Fusion.NewFusionNetwork import NewZedFusionNetworkDWSepRFB
        model = NewZedFusionNetworkDWSepRFB(
            cnn_model_cfg,
            trans_model_cfg
        ).cuda()
        ...
        # print(f'output.shape: {output.shape}')
        # exit(1)
    elif model_name == 'SingleTransformerZedFusion':
        # kind of like a smaller transformer repeated twice (so two small transformers)
        # where the input is from the CNN and reshaped not the actual images themselves
        from seg.model.Fusion.NewFusionNetwork import SingleTransformerZedFusion
        model = SingleTransformerZedFusion(
            cnn_model_cfg,
            trans_model_cfg,
        ).cuda()
        count_parameters(model)
        ...
        print(f'output.shape: {output.shape}')
        exit(1)
    elif model_name == 'NewZedFusionAttentionTransDecoderDWSepCNNWithCCMFuse':
        from seg.model.Fusion.NewFusionNetwork import NewZedFusionAttentionTransDecoderDWSepCNNWithCCMFuse
        model = NewZedFusionAttentionTransDecoderDWSepCNNWithCCMFuse(
            cnn_model_cfg,
            trans_model_cfg,
        ).cuda()
        count_parameters(model)
    ...
    elif model_name == 'ReverseAttentionCNNandlittleandBigTransformer':
        ...
        from seg.model.Fusion.NewFusionNetwork import ReverseAttentionCNNandlittleandBigTransformer
        torch.cuda.empty_cache()
        model = ReverseAttentionCNNandlittleandBigTransformer(
            cnn_model_cfg,
            big_trans_model_cfg,
            sml_trans_model_cfg,
            decoder_cfg,
    ...
    elif model_name == 'JustViT':
        from seg.model.Fusion.AblationStudiesScientificReports.NewFusionNetworkScientificReports import JustViT
        model = JustViT(
            trans_model_cfg=trans_model_cfg
        ).cuda()
    elif model_name == 'JustViTAndPartialDecoder':
        from seg.model.Fusion.AblationStudiesScientificReports.NewFusionNetworkScientificReports import JustViTAndPartialDec
        model = JustViTAndPartialDec(
            trans_model_cfg=trans_model_cfg
        ).cuda()
    else:
